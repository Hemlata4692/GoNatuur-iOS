...
@property (nonatomic, assign) int topSpace;
...
@property (nonatomic, strong) UIImageView *minRangeLabelImage;
@property (nonatomic, strong) UIImageView *maxRangeLabelImage;
...
- (void)setDefaultValues
{
    ...
    self.maxRangeLabelImage = [[UIImageView alloc] initWithFrame:CGRectMake(0, 0, 45, 26)];
     self.maxRangeLabelImage.center = CGPointMake([self sliderMidPointForPoint:CGRectGetMaxX(self.bounds)], CGRectGetMidY(self.bounds) + SLIDER_BUTTON_WIDTH-_topSpace);
    self.maxRangeLabelImage.image=[UIImage imageNamed:@"tooltip"];
    [self addSubview:self.maxRangeLabelImage];
    ...
    self.maxRangeLabel = [[UILabel alloc] initWithFrame:CGRectMake(0, 0, 45, 26)];
     self.maxRangeLabel.backgroundColor=[UIColor clearColor];
    self.maxRangeLabel.center = CGPointMake([self sliderMidPointForPoint:CGRectGetMaxX(self.bounds)], CGRectGetMidY(self.bounds) + SLIDER_BUTTON_WIDTH-72);
    ...
    if ([self.startSliderButton isSelected])
    {
        if ([self shouldStartButtonSlideForPoint:point])
        {
            [UIView animateWithDuration:0.1 animations:^{
                ...
                self.minRangeLabel.center = CGPointMake(CGRectGetMidX(self.startSliderButton.frame), CGRectGetMaxY(self.startSliderButton.frame) + (SLIDER_BUTTON_WIDTH / 2)-72);
                
                  self.minRangeLabelImage.center = CGPointMake(CGRectGetMidX(self.startSliderButton.frame), CGRectGetMaxY(self.startSliderButton.frame) + (SLIDER_BUTTON_WIDTH / 2)-_topSpace);
    ...
    else if ([self.endSliderButton isSelected])
    {
        if ([self shouldEndButtonSlideForPoint:point])
        {
            [UIView animateWithDuration:0.1 animations:^{
                ...
                self.maxRangeLabel.center = CGPointMake(CGRectGetMidX(self.endSliderButton.frame), CGRectGetMaxY(self.endSliderButton.frame) + (SLIDER_BUTTON_WIDTH / 2)-72);
                
                self.maxRangeLabelImage.center = CGPointMake(CGRectGetMidX(self.endSliderButton.frame), CGRectGetMaxY(self.endSliderButton.frame) + (SLIDER_BUTTON_WIDTH / 2)-_topSpace);
