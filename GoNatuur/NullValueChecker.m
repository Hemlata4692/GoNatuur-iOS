...
+ (NSMutableArray *)checkArrayForNullValue: (id)array{
    for (int i = 0; i < [array count]; i++) {
        id value;
        if ([array isKindOfClass:[NSDictionary class]]) {
            value = [array allValues][i];
            if ([value isKindOfClass:[NSNull class]] || value == (id)[NSNull null] || value == NULL || [value isEqual:@"<null>"]){
                [array setObject:@"" forKey:[array allKeys][i]];
                
            }
            else if ([value isKindOfClass:[NSDictionary class]]) {
                [array setObject:[NullValueChecker checkDictionaryForNullValue:[NSMutableDictionary dictionaryWithDictionary:(NSDictionary *)value]] forKey:[array allKeys][i]];
            }
            else if ([value isKindOfClass:[NSArray class]]) {
                [array setObject:[NullValueChecker checkArrayForNullValue:[NSMutableArray arrayWithArray:(NSArray *)value]] forKey:[array allKeys][i]];
            }
        }
        else {
            value = array[i];
